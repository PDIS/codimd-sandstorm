...
exports.singleNoteIndex = singleNoteIndex
...
async function newNote (req, res, next) {
  // Prevent create note more than one at single note per grain
  ...
  const noteId = (note && models.Note.encodeNoteId(note.id)) || null
  if (process.env.CMD_SINGLE_NOTE && noteId) {
...
async function singleNoteIndex (req, res, next) {
  ...
  if (req.isAuthenticated()) {
    //  Y => FindOrCreate Note
    if (noteId) res.redirect(config.serverURL + `/${noteId}`)
    else res.redirect(config.serverURL + '/new')
  } else if (enableAnonymousUser || header['x-sandstorm-user-id']) {
    //  N => sandstorm logged or enable anonymous user
    //      Y => redirect to login
